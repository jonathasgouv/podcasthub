...
function noFollow() {
  document.getElementById('endofpage').innerText = 'Parece que você não adicionou nenhum podcast :(\nAdicione alguns podcasts a essa tag para usar essa página'
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  console.log(window.location.href.split('tagname=')[1])
  tagname = window.location.href.split('tagname=')[1]
  tagname = atob(tagname)
	data = await fetch('https://podcasterbackend.podcaster.repl.co/getTagged?uid=' + uid + '&tagname=' + tagname);
  ...
  if (response.notag) {
	...
	mypodcasts = response;
  author_element.innerText = tagname
	generateHTML(mypodcasts);
	document.body.style.visibility = 'visible';
});
...
function openEpisode(episode) {
  ...
  ep = episode.getAttribute('data-src');
	location.replace('/views/episodespage/podcast.html#id=' + podcastid + '&ep=' + ep);
}
